...
//			};
			float[] vertices = { // Vertices for a face
					-2.0f, -1.0f, 0.0f, // 0. left-bottom-front
	...
	// Render the shape (Buat tembok belakang)
		public void draw_tembok_blk(GL10 gl) {
			...
			gl.glPushMatrix();
			gl.glTranslatef(0f, 0f, cubeHalfSize);
			gl.glBindTexture(GL10.GL_TEXTURE_2D, textureIDs[17]);
			gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, 0, 4);
			gl.glPopMatrix();
		...
		public void draw_atap(GL10 gl) {
			...
			gl.glPushMatrix();
			gl.glTranslatef(0f, 0f, cubeHalfSize);
			gl.glBindTexture(GL10.GL_TEXTURE_2D, textureIDs[16]);
			gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, 0, 4);
			gl.glPopMatrix();
	...
	public void loadTexture(GL10 gl, Context c) {
		gl.glGenTextures(18, textureIDs, 0); // Generate texture-ID array for 6
											// IDs
